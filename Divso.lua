...
-------------------------------------------------
-- TELEPORT HELPERS (Improved getBase)
-------------------------------------------------
local function getBase(targetPlayer)
	...
	-- Look through every Plot and check its Data.Owner ObjectValue
		...
		if plot.Name == "Plot" or plot:IsA("Model") or plot:IsA("Folder") then
			...
			if data then
				...
				if ownerValue and ownerValue:IsA("ObjectValue") then
					...
					-- Owner.Value is usually a string (player name)
					if typeof(owner) == "string" and owner == targetPlayer.Name then
						-- Found the correct plot
					...
					-- Fallback in case Owner.Value is sometimes a Player instance
